      ...
      <form method="POST" action="{{ route('devices.store') }}" class="space-y-4">
         ...
         <div>
            ...
            <select name="device_type" id="device_type" required
               ...
               <option value="esp32" {{ old('device_type') === 'esp32' ? 'selected' : '' }}>ESP32</option>
               <option value="esp8266" {{ old('device_type') === 'esp8266' ? 'selected' : '' }}>ESP8266</option>
               <option value="arduino" {{ old('device_type') === 'arduino' ? 'selected' : '' }}>Arduino</option>
               <option value="raspberry_pi" {{ old('device_type') === 'raspberry_pi' ? 'selected' : '' }}>Raspberry Pi</option>
               <option value="other" {{ old('device_type') === 'other' ? 'selected' : '' }}>Other</option>
            </select>
         </div>

         <div>
            <label for="device_identifier" class="block text-xs font-medium text-neutral-600 mb-1">Device Identifier</label>
            ...
            <p class="mt-1 text-[10px] text-neutral-400">Unique hardware identifier (MAC address, serial number, etc.)</p>
         </div>

         <div>
            <label for="location" class="block text-xs font-medium text-neutral-600 mb-1">Location <span class="text-neutral-400">(optional)</span></label>
         ...
         <div>
            <label for="description" class="block text-xs font-medium text-neutral-600 mb-1">Description <span class="text-neutral-400">(optional)</span></label>
